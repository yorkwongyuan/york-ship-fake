...
export const Upload:FC<UploadProps> = (props)=>{
  ...
  const {action,defaultFileLists,onProgress,onSuccess,onError,beforeUpload,onChange} = props
...
// post 上传方法
  const post = (file:File) => {
    ...
    axios.post(action,formData,{
      ...
      if(onError){
        updateFileLists(_file,{status:'error',error:err})
        onError(err,file)
      }
      if(onChange){
        onChange(file)
...
let newFileLists = fileLists.filter(item => item.uid !== file.uid)
  }
  // input的change事件
